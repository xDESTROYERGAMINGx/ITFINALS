...
class StudentModel
{
    ...
    {
        $stmt = $this->db->prepare("INSERT INTO student_subject (student_id, subject_id, status, created_at) VALUES (:student_id,:subject_id, 'pending', NOW())");
        $stmt->bindParam(':student_id', $studentId, PDO::PARAM_STR);
    ...
    {
        $stmt = $this->db->prepare("SELECT s.* FROM subject s
        JOIN student_subject ss ON s.subject_id = ss.subject_id
    ...
    {
        $stmt = $this->db->prepare("SELECT g.*, s.* FROM grading g
