...
<nav class="sidebar d-none d-lg-flex flex-column">
    ...
    <a href="/available-subjects/<?= $faculty['user_id'] ?>"><i class="bi bi-book"></i> Available Subjects</a>
    <a href="/faculty-subjects/<?= $faculty['user_id'] ?>"><i class="bi bi-person-lines-fill"></i> My Subjects</a>
    <a href="#" class="active"><i class="bi bi-pencil-square"></i> Edit Grade</a>
    <hr>
</nav>
...
<main class="main-content">
    ...
    <div class="card-glass mb-4">
        <h5><i class="bi bi-journal-text"></i> UPDATE GRADE</h5>
        <hr>
        <h6><?= htmlspecialchars($student['name']) ?></h6>
        <form method="POST" action="/faculty-grading/EditStudentGrade/<?= $faculty['user_id'] ?>/<?= $subject['code'] ?>/<?= $student['user_id'] ?>/edit">
            <div class="row">
